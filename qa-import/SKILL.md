...
- Valid HTTP(S) URLs with a host found anywhere in a row become `evidence_urls`. Scheme matching is case-insensitive; surrounding or unmatched trailing punctuation is removed without stripping balanced URL parentheses. A subsequent scheme splits the candidate only with a lexical delimiter such as an adjacent comma or semicolon, preserving nested redirect/callback values and schemes in paths. URLs are parser-canonicalized (including Unicode percent-encoding), then stably deduplicated in first-seen order.
